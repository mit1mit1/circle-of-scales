<script setup lang="ts">
	...
	const getNotePosition = (note: Note, noteIndex: number) => {
		return {
			x:
				circleCentre.x +
		...
		{#each [...notes] as note, index}
			<g>
				<circle
					...
					cx={getNotePosition(note, index).x}
					cy={getNotePosition(note, index).y}
					r={30}
					stroke="black"
					class={isInIonianScale(index, rootNoteIndex) ? 'visible' : 'hidden'}
				/>
				<text
					x={getNotePosition(note, index).x}
					y={getNotePosition(note, index).y}
					class={`svgNoteName ${
